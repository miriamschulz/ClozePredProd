...
df <- df[!df$UniqueID %in% c('4lchm0'), ]  # remove subject(s)
#summary(as.factor(df$UniqueID))
...
comp.targets$AnswerTime <- as.numeric(comp.targets$AnswerTime)
mean(comp.targets$AnswerTime)
comp.answertime <- comp.targets %>% filter(TargetAnswer %in% c("True", "False"))
hist(comp.answertime$AnswerTime,
     breaks=30,
     col='steelblue')
```
