...
dat_plot <- summarySE(dat_all,
                      ...
                      groupvars=c("Region", "ExpCondition",
                                  ...
                                  "WordPosition", "Word"),
                      na.rm=T)
# dat_plot$WordPosition <- as.factor(as.numeric(dat_plot$WordPosition))
...
for (i in 1:length(unique(dat_plot$Item))) {
# for (i in c(1, 2, 1001, 1002)) {
  ...
  ggsave(plot_title, plot = last_plot(), width=12, height=5, dpi=320)
}
```
...
# Plot:
ggplot(data=dat_plot,
       aes(TargetFreq, RT, fill=TargetFreq)) +
