...
power_analysis <- function(dat,
                           mean_rt, expectancy_fixed, task_fixed,
                           n_subj, n_items,
                           verbose=FALSE) {
  ...
  for (s in 1:length(n_subj)) {
    ...
    for (i in 1:length(n_items)) {
      ...
      for (e in 1:length(expectancy_fixed)) {
        ...
        if (verbose == TRUE) {
          cat("      Expectancy:", exp, " Task:", task,
              sep=" ", end="\n")
        }

        fix <- c(mean_rt, exp, task)

        # For each iteration, run several simulations for more stable results
        # for (j in 1:10) {
        for (j in 1:5) {  #TODO set to 10 for final simulations
...
Set the fixed and random effects, and iniialize:
...
expectancy_fixed <- c(5, 8, 10)  # different simulation values for expectancy
task_fixed <- c(5, 8, 10)  # different simulation values for task
...
### Power by number of subjects and items

Plot by number of subjects and items:
...
Plot by effect size (only the max number of items):
...
for (e in 1:length(expectancy_fixed)) {
  ...
  cat("Expectancy effect size:", exp_current, "-> min required subjects:",
      min_exp, sep=" ", end="\n")
  ...
  cat("Task effect size:", task_current, "      -> min required subjects:",
      min_task, sep=" ", end="\n\n")
}
